...
- サンプル: 3層構成 / Web3層 / 冗長化 / 本社-支店WAN / 拠点LAN / ハイブリッド /
  Spine-Leaf / 生成 123ノード（3×4 グリッド）
- キャンバス: パン・ズーム、ミニマップ、フィット表示
- **ノードをドラッグすると、つながっているエッジが追従**（未ドラッグのエッジは D2 のルートのまま）
...
- エッジは未ドラッグなら `route` の折れ線をそのまま描き、**ドラッグされたノードに接続する
  エッジだけ** React Flow の `getSmoothStepPath` で引き直す（`src/components/D2Edge.tsx`）。
  どのノードが動いたかは Context で持っている（`src/components/context.ts`）。
...
- ドラッグしたノードに接続するエッジは **smoothstep で引き直す**ため、そのぶん D2 の
  厳密なルートは失われる。元の D2 ルートへ戻すには再描画（`再描画` ボタン）する。
  編集後の座標を D2 に戻して再レイアウトする（tala の `top` / `left` で座標固定）のは未着手。
